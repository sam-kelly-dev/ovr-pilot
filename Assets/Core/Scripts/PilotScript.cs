...
public class PilotScript : MonoBehaviour
{
	...
	Transform hmdTarget;
		[SerializeField]
		Vector3 avatarHeadlookOffset = Vector3.zero;
		[SerializeField]
		Vector3 handRotationOffset = Vector3.zero; //This value should be tweaked on a per-model basis.

		// Use this for initialization
		void Start ()
		{
				CreateCharacter (defaultCharacter);
		Debug.Log ("HMDs detected:" + Ovr.Hmd.Detect ());
				if ((useVR && Ovr.Hmd.Detect () > 0) || forceVR) {
						InitializeVR ();
				} else {
						InitializeCam ();
				}
		...
		void InitializeVR ()
		{
				...
				hmd = (GameObject)GameObject.Instantiate (vrHeadCamera, head.position, Quaternion.identity);
				...
				hmdTarget = hmd.transform.Find ("CenterEyeAnchor/HeadIKTarget");
				hmdTarget.localRotation = head.localRotation;
				GameObject.Find ("DebugOverlay").gameObject.SetActive (false);
		}

		void InitializeCam ()
		{
				Debug.Log ("Initializing non-VR camera");
				Transform head = animator.GetBoneTransform (HumanBodyBones.Head);
